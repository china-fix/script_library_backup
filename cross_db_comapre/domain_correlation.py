"""
Hi there,
This is a hmmer3 dependent python script to correlate the proteins (references proteins/query proteins) according the pfam database.
...
def parse_args():
    ...
    parser.add_argument('--OUTPUT', default="domain_cor_out", type=str, metavar='directory', help="Output directory name")
...
def main():
    ...
    pandatab_RQ= pd.merge( pandatab_R, pandatab_Q, how= 'outer', on=['ACC', 'DESCRIPTION', 'HIT_ID', 'HIT_SEQ_LEN'])
    pandatab_RQ_inner= pd.merge( pandatab_R, pandatab_Q, how= 'inner', on=['ACC', 'DESCRIPTION', 'HIT_ID', 'HIT_SEQ_LEN'])
