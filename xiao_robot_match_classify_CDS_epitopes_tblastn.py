'''
This robot can help to locally tblastn the CDS.fasta to the reference epitops and filter the orginal CDS.fasta to matched_CDS.fasta and un_matched_CDS.fasta  
'''
...
def doing_blast(query, reference):
    subprocess.run(["tblastn", "-query", reference , "-subject", query, "-outfmt", "5", "-out", "temp_blast.xml", "-max_target_seqs", "13477"], check=True)
...
def filter_matching(cutoff):
    ...
    for blast_record in blast_records:
        ...
        for alignment in blast_record.alignments:
            ...
            if  hsp.identities / blast_record.query_length >= cutoff : # and hsp.expect <= 1e-10 : #and hsp.query_start == 1 and hsp.query_end == blast_record.query_letters:
                matched_result=alignment.title.split()[0]
                matched_list.append(matched_result)

                print(blast_record.query +'\t'+ alignment.title.split()[0]
...
def filter_matching_1(cutoff):
    ...
    for blast_record in blast_records:
        ...
        for alignment in blast_record.alignments:
            hsp = alignment.hsps[0]
            if  blast_record.query_length - hsp.identities <= cutoff : # and hsp.expect <= 1e-10 : #and hsp.query_start == 1 and hsp.query_end == blast_record.query_letters:
                matched_result=alignment.title.split()[0]
                matched_list.append(matched_result)
...
def main():
    args = parse_args()
    doing_blast(query=args.CDS, reference=args.REF)
    print("##########################################")
    print("tblasn and get the xml passed")
    ...
    if args.ANN:
        import pandas as pd
        EPITOPE_ANN_DF=pd.read_csv(args.ANN_FILE, skiprows=1 , low_memory=False)
